...
```go
que.WithServerURL("https://api.example.com")
```
...
```go
que.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
que.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
que.WithTimeout(30 * time.Second)
```
